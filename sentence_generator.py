...
def build_Probabilistic_POS(posFile, transitions):
    pos_sequence_list = [] # the list of sequences to return
    POS_frequencies = defaultdict(list) # bigram to bigram frequency dictionary
    starting_bg = ""
    next_bg = ""
    starters = [] # list of starting bigrams
    next_is_start1 = False
    next_is_start2 = False
    starter = ""
    
    # find all starting POS bigrams from the part of speech text
    f = open(posFile).read()
    for word in f.split():
        tags = word.split("_")
        if len(tags) == 2:
            if tags[1] is "." or tags[1] is "!" or tags[1] is "?":
                next_is_start1 = True
                total_sequence = []
            elif next_is_start1 is True:
                if tags[0] in dontInclude or tags[1] in dontInclude:
                    next_is_start1 = False
                starter += tags[1]
                next_is_start1 = False
                next_is_start2 = True
            elif next_is_start2 is True:
                if tags[0] in dontInclude or tags[1] in dontInclude:
                    next_is_start2 = False
                starter += "_" + tags[1]
                ...
                starter = ""
                next_is_start2 = False
    # build the bigram to bigram frequency dictionary, maps bigrams to list of
    # succeeding bigrams
    ...
    # select random bigrams from the starting bigram successor list and continue
    # until sentenceCap is reached
    for j in range(0, 10):
    #CHOOSE A RANDOM STARTING KEY
        ...
        sentenceCap = 20 # maximum length of the sentence
    ...
    for pos in pos_sequence:
        ...
        if pos not in posEmissions:            
            ...
            if len(pos.split('_')) == 2:
                if pos in bgPosEmissions:
                    if len (bgPosEmissions[pos][0].word.split('_')) > 0:
                        for word in bgPosEmissions[pos][0].word.split('_'):
                            wordsToAdd.append(word)

            if len(pos.split('_')) == 1:
                if pos in uniPosEmissions:
                    if len (uniPosEmissions[pos][0].word.split('_')) > 0:
...
if __name__=='__main__':
    ...
    if int(pos_type) == 1:
        ...
        randomly_chosen_pos_sequences = build_Probabilistic_POS("./data/" + sys.argv[1] + "/pos.txt", "./data/" + sys.argv[1] + "/bg_transitions.txt")
    #----------------
